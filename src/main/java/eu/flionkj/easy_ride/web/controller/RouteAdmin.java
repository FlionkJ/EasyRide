...
public class RouteAdmin {
    ...
    public ResponseEntity<DefaultResponse> createStoppingPoint(@RequestBody CreateStoppingPointRequest request) {
        ...
        return switch (result) {
            case ALREADY_EXISTS -> new ResponseEntity<>(
                    new DefaultResponse("A stopping point with this name already exists."),
                    HttpStatus.CONFLICT
            );
            case NAME_IS_EMPTY -> new ResponseEntity<>(
                    new DefaultResponse("Name cannot be empty."),
                    HttpStatus.BAD_REQUEST
            );
            case CREATED_SUCCESSFULLY -> new ResponseEntity<>(
                    new DefaultResponse("Stopping point was created successfully."),
                    HttpStatus.CREATED
            );
        };
    }
    ...
    public ResponseEntity<DefaultResponse> createConnection(@RequestBody CreateConnectionRequest request) {
        ...
        return switch (result) {
            case ALREADY_EXISTS -> new ResponseEntity<>(
                    new DefaultResponse("A connection with this parameters already exist."),
                    HttpStatus.CONFLICT
            );
            case START_IS_EMPTY -> new ResponseEntity<>(
                    new DefaultResponse("Start cannot be empty."),
                    HttpStatus.BAD_REQUEST
            );
            case START_POINT_NOT_FOUND -> new ResponseEntity<>(
                    new DefaultResponse("Start Point does not exist."),
                    HttpStatus.NOT_FOUND
            );
            case END_IS_EMPTY -> new ResponseEntity<>(
                    new DefaultResponse("End cannot be empty."),
                    HttpStatus.BAD_REQUEST
            );
            case END_POINT_NOT_FOUND -> new ResponseEntity<>(
                    new DefaultResponse("End Point does not exist."),
                    HttpStatus.NOT_FOUND
            );
            case AVERAGE_TRAVEL_TIME_MINUTES_IS_0 -> new ResponseEntity<>(
                    new DefaultResponse("Average travel time must be higher than 0."),
                    HttpStatus.BAD_REQUEST
            );
            case CREATED_SUCCESSFULLY -> new ResponseEntity<>(
                    new DefaultResponse("Connection was created successfully."),
                    HttpStatus.CREATED
            );
        };
    }
